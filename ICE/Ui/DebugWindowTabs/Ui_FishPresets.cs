...
namespace ICE.Ui.DebugWindowTabs
{
    internal class Ui_FishPresets
    {
        ...
        {
            if (ImGui.BeginTable(T("Fish Editor | Window Selector"), 2, ImGuiTableFlags.Borders | ImGuiTableFlags.SizingFixedFit, ImGui.GetContentRegionAvail()))
            {
                ...
                ImGui.InputUInt(T("Search ID"), ref search_MissionId);
                using (var missionSelection = ImRaii.Child(T("Mission Selection Child"), new(300, ImGui.GetContentRegionAvail().Y)))
                {
                    ImGui.Separator();
                ...
                using (var missionDetails = ImRaii.Child(T("Mission Selection Info"), ImGui.GetContentRegionAvail()))
                {
        ...
        private static void Mission_DetailedView()
        {
            if (CosmicHelper.SheetMissionDict.TryGetValue(selectedMission, out var missionInfo))
            {
                ...
                ImGui.Text(T("Current Mission: {0}", $"{selectedMission} {missionInfo.Name}"));
                ...
                if (ImGui.BeginTable(T("Autohook Presets"), 3, ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg))
                {
