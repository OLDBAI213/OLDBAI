            ...
            if (ImGui.BeginTable("Fishing Editor Table", 2, ImGuiTableFlags.Resizable | ImGuiTableFlags.SizingFixedFit))
            {
                ...
                if (ImGui.BeginChild(T("Fishing Hole Editor"), new Vector2(0, 0), true))
                {
                    if (selectedZone != 0 && selectedFlag != Vector2.Zero)
                    {
                        ...
                        ImGui.SetNextItemWidth(200);
                        if (ImGui.ColorEdit4(T("Cone Color Editor") + "##ConeColorEditor", ref coneColor))
                        {
                        ...
                        for (int i = 0; i < fishingHole.Count; i++)
                        {
                            ...
                            if (ImGui.Selectable(T("Spot {0}", i + 1), isSelected))
                            {
                                selectedSpotIndex = i;
                            }
                        ...
                        if (selectedSpotIndex >= 0 && selectedSpotIndex < fishingHole.Count)
                        {
                            ...
                            ImGui.SetNextItemWidth(200);
                            if (ImGui.InputFloat3(T("Fishing Position"), ref fish))
                            {
                            ...
                            ImGui.SetNextItemWidth(200);
                            if (ImGui.InputFloat3(T("Nav Position"), ref nav))
                            {
                            ...
                            if (ImGui.SliderFloat(T("Rotation Tolerance (degrees)"), ref toleranceDegrees, 1f, 45f))
                            {
                        ...
                        using (var drawList = PctService.Draw())
                        {
                            if (viewAllFishingSpots)
                            {
                                foreach (var location in fishingHole)
                                {
                                    ...
                                    drawList.AddText(floatPos, 2667577343, T("Spot #: {0}", holeNumber), 1);
                                    holeNumber+= 1;
                                }
                            }
                            if (viewNavSpot)
