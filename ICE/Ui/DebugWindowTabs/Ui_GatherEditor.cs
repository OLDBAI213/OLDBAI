...
namespace ICE.Ui.DebugWindowTabs
{
    internal class Ui_GatherEditor
    {
        ...
        public static void Draw()
        {
            ...
            ImGui.Text(T("Total: {0}", count));
            ImGui.SameLine();
            if (ImGui.Button(T("Set Save Location")))
            {
            ...
            {
                ImGui.TableSetupColumn(T("Route Selector"), ImGuiTableColumnFlags.WidthFixed, 200);
                ImGui.TableSetupColumn(T("Route Editor"), ImGuiTableColumnFlags.WidthStretch);
        ...
        private static void RouteSelector()
        {
            ...
            ImGui.InputTextWithHint("##routeSearch", T("Search..."), ref _routeSearch, 64);
        ...
        private static void RouteInfo()
        {
            ...
            {
                Utils.SetGatheringRing(mapInfo.TerritoryId, mapInfo.X, mapInfo.Y, mapInfo.Radius, T("Route {0}", _selectedRoute));
            }
            if (ImGui.IsItemHovered())
            {
                ImGui.BeginTooltip();
                ImGui.Text(T("Territory: {0}", mapInfo.TerritoryId));
                ImGui.Text(T("Location: {0}, {1}", mapInfo.X, mapInfo.Y));
                ImGui.Text(T("Radius: {0}", mapInfo.Radius));
                ImGui.EndTooltip();
            }
            ImGui.SameLine();
            ...
            if (GatheringRouteLoader.LoadedRoutes.TryGetValue(_selectedRoute, out var routeInfo))
            {
                ...
                if (ImGui.BeginChild("Node Selection", new(200, 200), true))
                {
                    if (Player.Available)
                    {
                        if (Svc.Objects.LocalPlayer.TargetObject != null)
                        {
                            ...
                            {
                                if (ImGui.Button(T("Add Node: {0}", lastTarget.BaseId)))
                                {
                ...
                {
                    ImGui.Text(T("Node: {0}", nodeInfo.NodeId));
                    ...
                    ImGui.Dummy(new(0, 5));
                    if (ImGui.Button(T("Player Start: {0}", nodeInfo.LandZone)))
                    {
                    ...
                    if (ImGui.DragFloat(T("Fan Start"), ref fanStart, 1, 0, 360))
                    {
                        nodeInfo.RadiusStart = fanStart;
                    }
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(100);
                    if (ImGui.DragFloat(T("Fan End"), ref fanEnd, 1, 0, 360))
                    {
                        nodeInfo.RadiusEnd = fanEnd;
                    }

                    ImGui.SetNextItemWidth(100);
                    if (ImGui.DragFloat(T("Min Distance"), ref fanMin, 1, 1, 4))
                    {
                    ...
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(100);
                    if (ImGui.DragFloat(T("Max Distance"), ref fanMax, 1, 1, 4))
                    {
                        nodeInfo.MaxDistance = fanMax;
                    }

                    ImGui.SetNextItemWidth(100);
                    if (ImGui.DragFloat(T("Fan Height"), ref height, 0.1f, 0, 3))
                    {
                ...
                if (ImGui.ColorEdit4(T("Gather Fan"), ref gatherFan))
                {
                    C.Picto_GatherFan = gatherFan;
                    C.SaveDebounced();
                }
                var selectedFan = C.Picto_SelectedFan;
                if (ImGui.ColorEdit4(T("Selected Fan"), ref selectedFan))
                {
            ...
            {
                ImGui.Text(T("No route file exist. Do you want to create one?"));
                if (ImGui.Button(T("Create files")))
                {
