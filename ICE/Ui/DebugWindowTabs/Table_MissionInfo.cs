...
namespace ICE.Ui.DebugWindowTabs
{
    internal class Table_MissionInfo
    {
        ...
        public static unsafe void Draw()
        {
            ...
            if (ImGui.Button(T("Copy Missing CSV")))
            {
                ...
                if (count > 0)
                {
                    ...
                    statusMessage = T("Copied {0} missing MissionScores rows (bronze) to clipboard", count);
                }
                else
                    statusMessage = T("No missing rows - embedded CSV covers all missions with bronze scores.");
            }
            if (ImGui.IsItemHovered())
            {
                ImGui.BeginTooltip();
                ImGui.Text(T("Rows for missions not in MissionScores.csv, using BronzeScore from sheets."));
                ImGui.Text(T("Paste at end of Resources/MissionScores.csv"));
                ImGui.EndTooltip();
            }

            ImGui.SameLine();

            if (ImGui.Button(T("Copy Auxesia CSV")))
            {
                ...
                {
                    ImGui.SetClipboardText(text);
                    statusMessage = T("Copied {0} Auxesia rows to clipboard", count);
                }
                else
                    statusMessage = T("No missing Auxesia MissionScores rows.");
            }
            ...
            ImGui.SameLine();
            if (ImGui.Button(T("Export Missing CSV")))
            {
                if (string.IsNullOrWhiteSpace(exportPath))
                    statusMessage = T("Set export path first (or use Copy Missing CSV)");
            ...
            {
                ImGui.TextColored(
                    statusMessage.Contains("Success", StringComparison.OrdinalIgnoreCase) || statusMessage.Contains("成功") ? new System.Numerics.Vector4(0, 1, 0, 1) : new System.Numerics.Vector4(1, 0, 0, 1),
                    statusMessage
                );
            }
            ...
            if (ImGui.BeginTable("Moon Mission Information Table", 35, tableFlags)) // Increased column count by 1
            {
                ...
                for (int i = 1; i < 4; i++)
                {
                    ImGui.TableSetupColumn(T("Gather [{0}]", i));
                    ImGui.TableSetupColumn(T("Amount [G-{0}]", i));
                }
                ...
                foreach (var entry in CosmicHelper.SheetMissionDict)
                {
                    ...
                    {
                        if (ImGui.Button($"{T("Flag")}###Flag-{entry.Key}"))
                        {
                        ...
                        ImGui.SameLine();
                        if (ImGui.Button($"{T("Copy Flag")}##Flag-{entry.Key}"))
                        {
        ...
        private static void ExportToCsv()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(exportPath))
                {
                statusMessage = T("Error: Please specify an export path");
                    return;
                }
                ...
                statusMessage = T("Success: Exported {0} missions to {1}", CosmicHelper.SheetMissionDict.Count, exportPath);
            }
            catch (Exception ex)
            {
                statusMessage = T("Error: {0}", ex.Message);
            }
        }
