...
namespace ICE.Ui.MainUi.HelpFolder.Tips_Folder
{
    internal class ModeSelection
    {
        ...
        private static void StandardMode()
        {
            ...
            ImGui.TextWrapped(
                T("The most straightforward mode. Standard runs only the missions you have enabled for your current class — you pick what you want done, and it handles the rest."));
            ...
            ImGui.TextWrapped(
                T("Basic missions (Ranks D→A) will only pull from the class you started on. Provisional missions (Weather, Timed, and Sequence) and Red Alerts will be picked up between missions — enable the multi-class setting to allow switching classes for these as well."));
        }
        ...
        private static void RelicGrind()
        {
            ...
            ImGui.TextWrapped(
                T("A mode designed to automate mission selection for relic progression with minimal intervention. It scans all available missions, evaluates the experience each one provides, and picks whichever yields the most for your current level."));
            ImGui.TextWrapped(
                T("If you're high enough level to need the next rank category but haven't unlocked it yet (e.g. Rank D completed but Rank C not yet unlocked), make sure to unlock it manually before starting."));
