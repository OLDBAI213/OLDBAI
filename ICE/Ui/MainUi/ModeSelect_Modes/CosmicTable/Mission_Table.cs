...
namespace ICE.Ui.MainUi.ModeSelect_Modes.CosmicTable
{
    ...
    internal class Mission_Table : Table<MissionInfo>, IDisposable
    {
        ...
        public readonly NameColumn _nameColumn = new() { Label = T("Name") };
        public readonly IdColumn _idColumn = new() { Label = T("ID") };
        public readonly JobColumn _jobColumn = new() { Label = T("Job") };
        public readonly MissionColumn _missionColumn = new() { Label = T("Rank") };
        public readonly CompletionColumn _completionColumn = new() { Label = T("Status") };
        public readonly ClassScoreColumn _classScoreColumn = new() { Label = T("Class") };
        public readonly CosmocreditColumn _cosmoColumn = new() { Label = T("Cosmo") };
        public readonly LunarCreditColumn _lunarColumn = new() { Label = T("Lunar") };
        public readonly DroneCreditColumn _droneColumn = new() { Label = T("Dronebits") };
        public readonly PlanetTokensColumn _planetTokenColumn = new() { Label = T("Mount") };
        public readonly SPMColumn _spmColumn = new() { Label = T("SPM") };
        public readonly TurninColumn _turninColumn = new() { Label = T("Goal") };
        public readonly PlanetColumn _planetColumn = new() { Label = T("Moons") };
        public readonly ProfileColumn _profileColumn = new() { Label = T("Profile") };
        public readonly NotesColumn _notesColumn = new() { Label = T("Notes") };
        public readonly AllRelicExpColum _allExpColumn = new() { Label = T("Exp") };
        ...
        public sealed class NameColumn : VerticalCenterColumnString
        {
            ...
            public override void DrawColumn(MissionInfo mission, int _)
            {
                ...
                {
                    ImGuiEx.IconWithTooltip(FontAwesomeIcon.ExclamationTriangle, T("This mission is not currently supported\n" +
                ...
                {
                    ImGuiEx.IconWithTooltip(FontAwesomeIcon.ExclamationTriangle, T(
        ...
        public sealed class ProfileColumn : ItemFilterColumn
        {
            ...
            public override float Width => Math.Max(
                ...
                ImGui.CalcTextSize(T("Open Craft Settings")).X + ImGui.GetStyle().FramePadding.X * 2 + ImGui.GetStyle().CellPadding.X * 2
            );
            public override int Compare(MissionInfo lhs, MissionInfo rhs) => lhs.SheetInfo.Jobs.First().CompareTo(rhs.SheetInfo.Jobs.First());
            public override void DrawColumn(MissionInfo item, int idx)
            {
                ...
                if (sheetInfo.Attributes.HasFlag(MissionAttributes.Craft))
                {
                    ...
                    {
                        ImGui.OpenPopup($"{T("Craft Settings: Recipies")}##CraftSettings_{item.Id}");
                    }

                    if (ImGui.BeginPopup($"{T("Craft Settings: Recipies")}##CraftSettings_{item.Id}"))
                    {
                ...
                if (gatherProfile)
                {
                    if (!collectable)
                    {
                        ...
                        if (C.MissionConfig.TryGetValue(item.Id, out var config))
                        {
                            ...
                            {
                                ImGui.OpenPopup($"{T("Select Gather Profile")}##GatherProfile_{item.Id}");
                            }
                            if (ImGui.IsItemHovered())
                            {
                                ImGui.SetTooltip(T("Select gathering profile"));
                            }
                            if (ImGui.BeginPopup($"{T("Select Gather Profile")}##GatherProfile_{item.Id}"))
                            {
                ...
                else if (sheetInfo.Attributes.HasFlag(MissionAttributes.Fish))
                {
                    if (C.MissionConfig.TryGetValue(item.Id, out var config))
                    {
                        ...
                        {
                            ImGui.OpenPopup($"{T("Select Fishing Profile")}##FishingProfile_{item.Id}");
                        }
                        if (ImGui.BeginPopup($"{T("Select Fishing Profile")}##FishingProfile_{item.Id}"))
                        {
